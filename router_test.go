...
func TestRouter(t *testing.T) {
	Convey("Given a new fiber app and a new generator", t, func() {
		...
		Convey("When adding a JSON response", func() {
			...
			Convey("The response for status code 200 should be added", func() {
				...
				response := openapi3.NewResponse().
					...
					WithDescription("OK")
				So(operation.Responses.Status(200).Value.Description, ShouldEqual, response.Description)
				So(operation.Responses.Status(200).Value.Content["application/json"].Schema.Value.Type, ShouldEqual, response.Content["application/json"].Schema.Value.Type)
			})
		})
		...
		Convey("When adding a hook after bind", func() {
			var hookedValue int
			hook := func(c *gin.Context, in any) {
				hookedValue++
			}
		...
		Convey("When creating a group", func() {
			...
			Convey("The handler should work", func() {
				...
				Convey("The middlewares should work", func() {
					value := 0
					addValueMiddleware := func(c *gin.Context) {
						value++
						c.Next()
					}
